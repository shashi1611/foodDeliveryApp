...
import com.restaurant.fooddeliveryapp.addProductSoup;
import com.restaurant.fooddeliveryapp.models.modelSoup;

import java.util.List;

public class adapterSoup extends RecyclerView.Adapter<adapterSoup.MyViewHolder> {
    Context context;
    List<modelSoup> list;
    public adapterSoup(Context cont,List<modelSoup>li)
    {
        this.context=cont;
        this.list=li;
    }
    ...
    @Override
    public adapterSoup.MyViewHolder onCreateViewHolder(@NonNull @org.jetbrains.annotations.NotNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.single_item_all_product,parent,false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @org.jetbrains.annotations.NotNull adapterSoup.MyViewHolder holder, int position) {
        modelSoup m=list.get(position);
...
//        holder.text2.setText(list.get(position).getLoc());
        ...
        holder.btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {
                    Intent intent=new Intent(context, addProductSoup.class);
